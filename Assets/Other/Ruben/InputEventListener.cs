...
public class Key
{
    ...
    public Func<List<KeyCode>> callback;
...
public class InputEventListener : MonoBehaviour
{
    ...
    public void listenTo(KeyCode keyCode,Func<List<KeyCode>> callback)
    {
        Key key = new Key(keyCode);
        key.callback = callback;
        keys.Add(key);
    }
    public void listenTo(KeyCode keyCode,Func<List<KeyCode>> callback, Func<KeyCode,bool> call)
    {
    ...
    public void scan()
    {
        ...
        void FixedUpdate()
        {
            foreach(Key key in keys)
            {
                if (key.state)
                {
                    List<KeyCode> attr = new List<KeyCode>();
                    attr.Add(key.code);
